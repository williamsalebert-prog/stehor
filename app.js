(() => {
  ...
  function putEntity(entity){
    return new Promise((resolve,reject)=>{
      const r=tx(STORE,"readwrite").put(entity); r.onsuccess=()=>resolve(entity); r.onerror=()=>reject(r.error);
    });
  }
  ...
  function navigateTo(view,push=true){
    if(push) pushHistory();
    if(view.type==="home") showHome(false);
    else if(view.type==="category") showCategory(view.id,false);
    else if(view.type==="registry") showRegistry(false);
    else if(view.type==="linker") showLinker(false);
    else if(view.type==="entity") showEntity(view.id,false);
  }
  ...
  function buildNav(){
    ...
    for(const [id,name] of window.SETHORIA_DEMO.categories){
      ...
      b.dataset.nav=id;
      b.innerHTML=`<span class="cat-icon">${categoryIcons[id]||"•"}</span><span>${esc(name)}</span>`;
      b.onclick=()=>showCategory(id,true);
      nav.appendChild(b);
    }
  }

  function homeCategoryCard(id,name){
    const count=entities.filter(e=>e.category===id).length;
    return `<button class="category-card" data-home-category="${id}">
      <span class="count">${count}</span>
      <div class="category-icon">${categoryIcons[id]||"•"}</div>
      <strong>${esc(name)}</strong>
      <small>${count ? `${count} elemento${count===1?"":"s"}` : "Vacío"}</small>
    </button>`;
  }

  function showHome(push=true){
    if(push) pushHistory();
    currentView={type:"home"}; setActive("home"); setBreadcrumb("Inicio");
    $("#view").innerHTML=`
      <div class="home-head">
        <div class="eyebrow">Atlas personal de Sethoria</div>
        <h1>Inicio</h1>
        <p>Busca, abre una categoría o continúa organizando información. La interfaz todavía es prototipo; el contenido técnico seguirá creciendo por módulos.</p>
      </div>
      ...
      <div class="category-grid">
        ${window.SETHORIA_DEMO.categories.map(([id,name])=>homeCategoryCard(id,name)).join("")}
      </div>

      <div class="section-head">
        <div>
          <h2>Herramientas</h2>
          <small>Funciones transversales</small>
        </div>
      </div>
      <div class="quick-row">
        <button class="quick-card" id="homeRegistry">
          <strong>Registro maestro</strong>
          <small>IDs, etiquetas maestras, alias y referencias.</small>
        </button>
        <button class="quick-card" id="homeLinker">
          <strong>Hipervinculador rápido</strong>
          <small>Pega texto y revisa coincidencias antes de vincular.</small>
        </button>
        <button class="quick-card" id="homeBackup">
          <strong>Respaldo local</strong>
          <small>Exporta el estado actual cuando quieras conservar una copia.</small>
        </button>
      </div>
    `;
  ...
  function showCategory(id,push=true){
    if(push) pushHistory();
    currentView={type:"category",id}; setActive(id); setBreadcrumb(categoryName(id));
    const list=entities.filter(e=>e.category===id);
    $("#view").innerHTML=`
      <div class="page-title">
        <div>
          <h1>${esc(categoryName(id))}</h1>
          <p>${categoryDescription(id)}</p>
        </div>
        <button class="primary-button" id="categoryNew">＋ Nuevo</button>
      </div>
      ${list.length
        ? `<div class="grid">${list.map(entityCard).join("")}</div>`
        : `<div class="empty">Todavía no hay elementos aquí. Usa “Nuevo” cuando quieras empezar.</div>`}
    `;
    $("#categoryNew").onclick=()=>openDialog(id);
    $$("[data-entity-card]").forEach(c=>c.onclick=()=>showEntity(c.dataset.entityCard,true));
  }

  function categoryDescription(id){
    const descriptions = {
      personajes:"Personajes, perfiles, evolución y conexiones.",
      grupos:"Grupos, subgrupos, tripulaciones y facciones.",
      arcos:"Arcos, tramas y subtramas.",
      eventos:"Acontecimientos concretos y sus conexiones.",
      ...
      rutas:"Recorridos que no encajen cómodamente dentro de un lugar.",
      batallas:"Técnicas de combate y batallas específicas.",
      barcos:"Barcos, planos, capacidades, tripulaciones y viajes.",
      "criaturas-magicas":"Criaturas fantásticas o sobrenaturales.",
      "criaturas-reales":"Animales reales, extintos o prehistóricos.",
      culturas:"Culturas, sociedades y estructuras sociales.",
      armas:"Armas y datos técnicos relacionados.",
      ropa:"Ropa, atuendos, materiales y uso.",
      objetos:"Objetos y equipo que no pertenecen a armas o ropa.",
      relaciones:"Relaciones personales y sentimentales.",
      "historia-real":"Hechos históricos tal como ocurrieron.",
      reglas:"Reglas canónicas del funcionamiento de Sethoria.",
      "informacion-general":"Información transversal que no necesita módulo propio.",
      musica:"Música, referencias, inspiración y material asociado.",
      fuentes:"Fuentes y archivos de investigación.",
      otros:"Colecciones auxiliares o información complementaria.",
      notas:"Ideas, pendientes, dudas y notas libres."
    };
    return descriptions[id] || "Vista de categoría.";
  }

  function entityCard(e){
    return `<button class="entity-card" data-entity-card="${e.id}">
  ...
  function showRegistry(push=true){
    if(push) pushHistory();
    currentView={type:"registry"}; setActive("registry"); setBreadcrumb("Registro maestro");
    const rows=entities.slice().sort((a,b)=>a.title.localeCompare(b.title,"es"));
    $("#view").innerHTML=`
      <div class="page-title">
        <div>
          <h1>Registro maestro</h1>
          <p>Índice técnico de entidades. Está colocado como herramienta, no como parte central de la navegación.</p>
        </div>
      </div>
  ...
  function detectText(text){
    ...
    for(const e of entities){
      for(const info of termsForEntity(e)){
        const term=String(info.term).trim(); if(!term) continue;
        let start=0; const tLower=term.toLocaleLowerCase("es");
  ...
  function showLinker(push=true){
    if(push) pushHistory();
    currentView={type:"linker"}; setActive("linker"); setBreadcrumb("Hipervinculador rápido");
    $("#view").innerHTML=`
      <div class="page-title">
        <div>
      ...
      <div class="two-col">
        <div class="panel" style="margin-top:0">
          ...
          <div class="copy-group" style="margin-top:10px">
  ...
  function runDetection(){
    ...
    list.innerHTML=linkerState.map((m,i)=>{
      ...
      for(const c of m.candidates){ if(!seen.has(c.entity.id)){ seen.add(c.entity.id); unique.push(c); } }
      m.candidates=unique;
      return `<div class="detected-item ${unique.length>1?"ambiguous":""}">
        <div class="row">
          <div><strong>${esc(m.text)}</strong><div class="meta">${unique.length>1 ? `${unique.length} candidatos` : unique[0].source}</div></div>
  ...
  function renderLinkedPreview(text){
    ...
    $$("[data-entity-link]").forEach(a=>a.onclick=(ev)=>{ev.preventDefault();showEntity(a.dataset.entityLink,true);});
  }
    ...
    $("#view").innerHTML=`
      <div class="entity-hero">
        <div>
          ...
          <p style="line-height:1.65;color:#8997aa">${esc(e.summary||"Sin resumen.")}</p>
      ...
      <div class="notice">Esta ficha sigue siendo de arquitectura. Las vistas especializadas vendrán después de validar la navegación.</div>`;
  ...
  function openDialog(defaultCategory="personajes"){
    ...
    $("#entityTitle").value=""; $("#entityMasterTag").value=""; $("#entityMasterTag").dataset.touched="";
    $("#entityAliases").value=""; $("#entityTags").value=""; $("#entitySummary").value="";
    $("#entityDialog").showModal(); $("#entityTitle").focus();
  }

  async function saveForm(){
    const title=$("#entityTitle").value.trim(), category=$("#entityCategory").value;
    let masterTag=$("#entityMasterTag").value.trim();
    if(!masterTag) masterTag=`${slug(category)}_${slug(title)}`;
    if(entities.some(e=>e.masterTag===masterTag)){ alert("La etiqueta maestra ya existe."); return false; }
    const e={id:uid(),title,category,masterTag,
    ...
    await putEntity(e); entities=await getAll(); $("#entityDialog").close(); showCategory(category,true); return true;
  }

  function searchEntities(q){
    const term=q.trim().toLocaleLowerCase("es");
    if(!term) return entities.slice(0,10);
  ...
  function renderPalette(q){
    ...
    $$("[data-palette-id]").forEach(b=>b.onclick=()=>{closePalette();showEntity(b.dataset.paletteId,true);});
  }
  ...
  async function copyText(text){
    ...
    $("#storageStatus").innerHTML='<span class="status-dot"></span> Copiado';
    setTimeout(()=>$("#storageStatus").innerHTML='<span class="status-dot"></span> Datos locales activos',1000);
  }

  async function exportBackup(){
    const payload={format:"sethoria-atlas-prototipo-a1",version:2,exportedAt:new Date().toISOString(),entities};
  ...
  async function importBackup(file){
    ...
    await clearEntities(); for(const e of payload.entities) await putEntity(e);
    entities=await getAll(); showHome(true);
  }

  async function init(){
    buildNav(); await openDB(); entities=await ensureSeed();

    $("#homeBtn").onclick=()=>showHome(true);
    $("#brandHome").onclick=()=>showHome(true);
    $("#topHomeBtn").onclick=()=>showHome(true);
    $("#backBtn").onclick=goBack;
    $("#registryBtn").onclick=()=>showRegistry(true);
    $("#linkerBtn").onclick=()=>showLinker(true);
    $("#newEntityBtn").onclick=()=>openDialog();
    $("#quickSearchBtn").onclick=()=>openPalette();
    ...
    $("#entityForm").addEventListener("submit",async ev=>{ev.preventDefault();await saveForm();});
    ...
    $("#commandPalette").onclick=e=>{ if(e.target===$("#commandPalette")) closePalette(); };
    $("#paletteSearch").oninput=e=>{paletteIndex=0;renderPalette(e.target.value);};
    $("#paletteSearch").onkeydown=e=>{
      if(e.key==="ArrowDown"){e.preventDefault();paletteMove(1);}
      else if(e.key==="ArrowUp"){e.preventDefault();paletteMove(-1);}
      else if(e.key==="Enter" && paletteItems[paletteIndex]){e.preventDefault();const id=paletteItems[paletteIndex].id;closePalette();showEntity(id,true);}
    ...
    document.addEventListener("keydown",e=>{
      ...
      else if(!typing && e.key.toLowerCase()==="n"){e.preventDefault();openDialog();}
      else if(e.altKey && e.key==="ArrowLeft"){e.preventDefault();goBack();}
      else if(e.key==="Escape" && !$("#commandPalette").classList.contains("hidden")) closePalette();
    });
